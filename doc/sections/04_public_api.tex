...
\item \texttt{<oqs/sig.h>}: 전자서명 객체/함수
\end{itemize}
...
\subsection{SIG API 흐름}
\begin{enumerate}[leftmargin=*]
\item \texttt{OQS\_SIG\_alg\_identifier(i)} 또는 문자열 상수로 알고리즘 선택
\item \texttt{OQS\_SIG\_new(name)}로 객체 생성
\item 길이 필드(\texttt{length\_public\_key}, \texttt{length\_signature} 등) 기반 버퍼 할당
\item \texttt{OQS\_SIG\_keypair}, \texttt{OQS\_SIG\_sign}, \texttt{OQS\_SIG\_verify} 호출
\item \texttt{OQS\_SIG\_free()} 및 민감정보 secure free
\end{enumerate}
...
\texttt{OQS\_KEM\_alg\_count()}, \texttt{OQS\_SIG\_alg\_count()}와 각 \texttt{alg\_identifier()}로 열거되는 목록이
실행 시점의 enabled 상태와 동일하지 않을 수 있으므로,
실사용 시 \texttt{OQS\_KEM\_alg\_is\_enabled()} 또는 \texttt{OQS\_SIG\_alg\_is\_enabled()} 체크를 병행한다.
